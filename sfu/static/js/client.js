...
let prevInboundStats = { bytes: 0, timestamp: 0 };   // для входящего видео от SFU
let statsHistory = {
  bitrateOutVideo: [],     // исходящий видео битрейт (kbps)
  bitrateOutAudio: [],     // исходящий аудио битрейт (kbps)
  ...
  signalingSocket.onmessage = async (event) => {
    ...
    switch (msg.type) {
      case 'joined':
        ...
        conferenceStartTime = Date.now();   // начало конференции
