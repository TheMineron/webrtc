...
async function collectFullStats() {
  ...
  try {
    ...
    if (outVideo) {
      ...
      localText += `🎥 Видео (исх.):\n   Битрейт: ${videoBitrate.toFixed(2)} kbps\n   Разрешение: ${width}×${height}\n   FPS: ${fps}\n   Отправлено пакетов: ${currentPackets}\n   Кодек: ${codec}\n`;
    } else {
    ...
    if (outAudio) {
      ...
      localText += `🎙️ Аудио (исх.):\n   Битрейт: ${audioBitrate.toFixed(2)} kbps\n   Отправлено пакетов: ${outAudio.packetsSent}\n`;
    } else {
    ...
    if (inboundVideo) {
      ...
      remoteText += `📥 Видео (вх.):\n   Битрейт: ${bitrateIn.toFixed(2)} kbps\n   Потеря пакетов: ${inboundVideo.packetsLost || 0}\n   Джиттер: ${jitterMs.toFixed(2)} мс\n   Декодировано кадров: ${inboundVideo.framesDecoded || 0}\n`;
    } else {
  ...
  try {
    ...
    sfuSocket.onmessage = async (event) => {
      ...
      if (msg.type === 'joined') {
        ...
        // Сервер инициирует renegotiation, отправляя offer
        ...
        try {
          ...
          sfuSocket.send(JSON.stringify({ type: 'answer', sdp: answer.sdp }));
          updateStatus('Renegotiation answer sent');
        } catch (err) {
          console.error('Failed to handle offer:', err);
        }
      } else if (msg.type === 'answer') {
        // Ответ на наш offer (начальный или renegotiation)
        try {
...
async function setupSFUPeerConnection() {
  ...
  sfuPeerConnection.onicecandidate = (event) => {
    if (event.candidate && event.candidate.candidate) { // фильтруем пустые кандидаты
